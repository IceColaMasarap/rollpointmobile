...
class _InstructorPageState extends State<InstructorPage> {
  ...
  Future<void> _loadUserData() async {
    try {
      ...
      final assignmentsResponse = await _supabase.rpc(
      ...
      final transformedAssignments = assignmentsResponse
          .map<Map<String, dynamic>>(
            (assignment) => {
              ...
              'platoons': {
                'id': assignment['platoon_id'],
                'name': assignment['platoon_name'],
              },
            },
          )
          .toList();
  ...
  Future<void> _loadSelectedAssignment() async {
    try {
      ...
      final userId = _supabase.auth.currentUser?.id;
      final savedAssignmentId = prefs.getString(
        'selected_assignment_id_$userId',
      );

      if (savedAssignmentId != null && userAssignments.isNotEmpty) {
        // Try to find the saved assignment in current assignments
        final savedAssignment = userAssignments
            .where((assignment) => assignment['id'] == savedAssignmentId)
            .firstOrNull;
  ...
  Future<void> _loadAttendanceData() async {
    ...
    try {
      ...
      print(
        'Fetching attendance data for company: $companyId, platoon: $platoonId',
      );
                                ...
                                children: [
                                  ...
                                  Expanded(
                                    child: _buildStatCard(
                                      ...
                                      value:
                                          '${attendanceStats['attendanceRate'].toStringAsFixed(0)}%',
